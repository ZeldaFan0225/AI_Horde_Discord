            ...
            if(!status.done) return {status, horde_data}
                ...
                const image_map = images.generations?.map((g, i) => {
                    ...
                    const embed = new EmbedBuilder({
                        ...
                        color: Colors.Blue,
                        description: `**Seed:** ${g.seed}\n**Model:** ${g.model}\n**Generated by** ${g.worker_name}\n(\`${g.worker_id}\`)${!i ? `\n**Prompt:** ${prompt}\n**Total Kudos Cost:** \`${images.kudos}\`${(generation_data.params?.n ?? 1) > 1 ? ` (\`${(images.kudos ?? 0)*(generation_data.params?.n ?? 1)}\` total)` : ""}` : ""}`,
                    })
    ...
    override async autocomplete(context: AutocompleteContext): Promise<any> {
        ...
        switch(option.name) {
            case "model": {
                ...
                const available = [{name: "Any Model", value: "YOLO"}, ...models.sort((a, b) => b.performance!-a.performance!).map(m => ({name: `${m.name} | Workers: ${m.count} | Performance: ${m.performance} | Queued: ${m.queued}`, value: m.name!}))].filter(v => !context.client.config.blacklisted_models?.includes(v.value))
                return await context.interaction.respond(available.filter(o => o.name.includes(option.value)).slice(0,25))
            }
            case "width":
            case "height": {
