...
export default class extends Modal {
    ...
    override async run(ctx: ModalContext): Promise<any> {
        ...
        if(ctx.client.config.remix?.blacklist_regex && new RegExp(ctx.client.config.remix.blacklist_regex, "i").test(prompt.replace(/[\u0300-\u036f]/g, ""))) return ctx.error({error: "Your prompt included one or more blacklisted words"})
